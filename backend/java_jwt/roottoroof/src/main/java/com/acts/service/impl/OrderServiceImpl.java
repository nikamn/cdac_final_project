...
public class OrderServiceImpl implements OrderService {
    ...
    HashMap<Integer,List<Order>> cancelOrders = new HashMap<>();
    ...
    public void cancelOrder(Integer orderId) {
        ...
        List<Order> list = cancelOrders.get(user.getId());
        if(list!=null) {
             list.add(order);
             cancelOrders.put(user.getId(), list);
        }
        else{
            list = new ArrayList<>();
            list.add(order);
        ...
        System.out.println(cancelOrders.get(user.getId()).get(0).getId());
    ...
    public List<Order> getAllCancelOrders() {
        ...
        User user = userService.getUserFromEmail(JwtUtils.getEmail());
        if(cancelOrders.containsKey(user.getId())){
            return cancelOrders.get(user.getId());
        }
        return null;
    }
